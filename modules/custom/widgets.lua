...
function widgets.add_prog_toggle(widget, prog, mod, button) -- {{{
	...
	widget:buttons(awful.util.table.join(
		...
		awful.button(mod, button, function()
			if pid then
...
function widgets.new_playerstatus() -- {{{
	...
	vicious.register(playerstatus, function(format, warg)
		local player_state = {
			["{volume}"] = 0,
			["{bitrate}"] = 0,
			["{elapsed}"] = 0,
		...
		for line in f:lines() do
			for player, key, value in string.gmatch(line, "([%w]+) ([%w]+:[%w]+)[%s]+(.*)$") do
				if known_players[player] ~= nil then
					if key == "xesam:album" then
						player_state["{Album}"] = value
					elseif key == "xesam:artist" then
						player_state["{Artist}"] = value
					elseif key == "xesam:title" then
						player_state["{Title}"] = value
					end
				end
			end
		end
		...
		return player_state
	end, function(playerwidget, args)
		local text
		...
		if state then
			if state == "Stop" then
				...
				if artist ~= "N/A" and title ~= "N/A" then
					text = artist .. " - " .. title
	...
	playerstatus:buttons(awful.util.table.join(
...
function widgets.new_volume() -- {{{
	...
	vicious.register(volume, vicious.widgets.volume, "<span fgcolor='cyan'>$1%$2</span>", 1, "Master")
